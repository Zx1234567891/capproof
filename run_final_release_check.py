...
def claims_evidence_rows() -> list[dict[str, Any]]:
    rows = [
        proven("Hermes local CapProof MCP parity", "artifact_reports/real_environment_validation_summary.json", "Stage 38REAL", "b881d996afe58dfc65ce7e00e7e321c51c108651", "python run_real_environment_validation.py --all --require-real --fail-if-gate-missing --report", "Controlled local Hermes MCP path."),
        proven("OpenCode local CapProof MCP parity", "real_agent_integrations/opencode_mcp_server/reports/real_opencode_deepseek_parity_summary.json", "Stage 40O-D", "b949d71bc7d5ac3fe29be7a75d104c3338a71b72", "python run_real_opencode_deepseek_mcp_parity.py --all --require-real --report", "Controlled local OpenCode MCP path."),
        proven("OpenClaw local CapProof MCP parity", "real_agent_integrations/openclaw_mcp_server/reports/real_openclaw_deepseek_parity_summary.json", "Stage 40C-D", "7d967ebe053e0a7b9e199e7540dbc30547c33411", "python run_real_openclaw_deepseek_mcp_parity.py --all --require-real --report", "Controlled local OpenClaw MCP path."),
        proven("All three use DeepSeek via DEEPSEEK_API_KEY", "artifact_reports/cleanroom_release_candidate_summary.json", "Stage 43RC", "0ab6e29", "python run_cleanroom_release_candidate.py --fresh-run --require-real --fail-if-gate-missing --clean-before-run --report", "DeepSeek is not safety TCB."),
        proven("All three use same standard CapProof MCP server", "artifact_reports/agent_parity_matrix.json", "Stage 41AP", "47e66f0877aec0bf06b45f930ca0cb36fcb1ef9c", "python run_agent_parity_matrix.py --report", "Same productized stdio MCP server path."),
        proven("tools/list and tools/call observed for all three", "artifact_reports/cleanroom_release_candidate_summary.json", "Stage 43RC", "0ab6e29", "python run_cleanroom_release_candidate.py --fresh-run --require-real --fail-if-gate-missing --clean-before-run --report", "Observed in tested local path."),
        proven("sandboxed local read/write/template subset", "artifact_reports/cleanroom_release_candidate_summary.json", "Stage 43RC", "0ab6e29", "python run_cleanroom_release_candidate.py --fresh-run --require-real --fail-if-gate-missing --clean-before-run --report", "No arbitrary filesystem access."),
        proven("DENY/ASK executor gate", "artifact_reports/cleanroom_release_candidate_summary.json", "Stage 43RC", "0ab6e29", "python run_cleanroom_release_candidate.py --fresh-run --require-real --fail-if-gate-missing --clean-before-run --report", "DENY/ASK executor remains off."),
        proven("trusted ASK approve rerun", "artifact_reports/cleanroom_release_candidate_summary.json", "Stage 43RC", "0ab6e29", "python run_cleanroom_release_candidate.py --fresh-run --require-real --fail-if-gate-missing --clean-before-run --report", "Trusted local CLI only."),
        proven("LLM/MCP metadata cannot mint capability in tested paths", "artifact_reports/final_claims_evidence_index.json", "Stage 42EVAL", "d80e92e", "python run_real_agent_parity_evaluator.py --all --fresh-run --require-real --fail-if-gate-missing --report", "Tested metadata/natural-language rejection."),
        proven("clean-room fresh-run reproduction passed", "artifact_reports/cleanroom_release_candidate_summary.json", "Stage 43RC", "0ab6e29", "python run_cleanroom_release_candidate.py --fresh-run --require-real --fail-if-gate-missing --clean-before-run --report", "Release candidate reproduced from clean worktree."),
    ]
    rows.extend(
        not_claimed(name)
        for name in (
...
ALLOW_CAPROOF_REAL_OPENCLAW_SMOKE=1 \
DEEPSEEK_API_KEY="$DEEPSEEK_API_KEY" \
python run_final_release_check.py --fresh-run --require-real --fail-if-gate-missing --check-claims --check-secrets --check-forbidden-paths --check-checksums --report
```
"""
