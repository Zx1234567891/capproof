...
HERMES_SOURCE_SURFACES: tuple[dict[str, Any], ...] = (
    {
        ...
        "missing": ("tool_request compatibility shape", "enabled_toolsets", "turn_id", "api_request_id"),
        "update": "Stage 20 adds dispatcher_tool_call effective_args coverage; real tool_request capture still needs runtime field mapping.",
        "test": "Dispatcher effective_args is preferred over original_args and middleware rewrites are recorded.",
        "confidence": "high",
    },
    {
        ...
        "missing": ("old_string", "new_string", "replace_all", "session_id", "full patch semantics"),
        "update": "Stage 20 maps edit_file path/resolved_path/cross_profile/patch_ref to write_file or fail-closed; full patch semantics remain partial.",
        "test": "Hermes edit_file AGENTS.md without cap denies; authorized workspace edit allows.",
        "confidence": "high",
    },
    {
        ...
        "missing": ("background", "timeout", "pty", "notify_on_complete", "watch_patterns"),
        "update": "Stage 20 maps terminal.command to allowlisted run_shell templates or denies arbitrary commands; process-control fields remain partial.",
        "test": "Hermes terminal raw curl pipe denies; raw pytest maps to run_shell template with caps.",
        "confidence": "high",
    },
    {
        ...
        "missing": ("emoji", "message_id", "media local_path", "thread_id", "reaction action semantics"),
        "update": "Stage 20 canonicalizes target into platform/channel/recipient and maps message to body; media/reaction variants remain partial.",
        "test": "Hermes send_message target unauthorized denies NoCap; authorized target allows.",
        "confidence": "high",
    },
    {
        ...
        "missing": ("inputSchema", "transport.command", "resources/prompts", "non-http dynamic tools"),
        "update": "Stage 20 maps dynamic http_post MCP server/tool/arguments/transport endpoint/header fields; non-http tools remain partial.",
        "test": "Hermes dynamic MCP http_post preserves endpoint fields and unauthorized endpoint denies.",
        "confidence": "high",
    },
    {
        ...
        "missing": ("session_key", "permission id", "decision", "approval/control capability"),
        "update": "Stage 20 maps messages_send-like target/message through send_message; permissions_respond remains unmodeled.",
    ...
    },
    {
        ...
        "missing": ("old_text", "operations", "provider metadata", "session_id"),
        "update": "Stage 20 maps memory action/target/content/origin/persistent/scope to memory_write with authority stripping; operation diffs remain partial.",
        "test": "Hermes memory action with authority-like content strips authority and does not mint caps.",
        "confidence": "high",
    },
    {
        ...
        "missing": ("memory_type/category", "importance", "metadata", "remote container"),
        "update": "Stage 20 routes retaindb/supermemory/openviking-style memory provider tools through Memory Authority Stripping.",
    ...
    },
    {
        ...
        "missing": ("tasks", "acp_command", "acp_args", "parent_agent_id", "child_task_id", "non-email requested actions"),
        "update": "Stage 20 maps delegate_task parent/child/goal/context/toolsets to delegation request evidence; non-email actions remain partial.",
        "test": "Hermes delegate_task without Delegation Certificate denies DelegationMissing and amplification denies.",
        "confidence": "high",
    },
    {
    ...
    },
    {
        ...
        "missing": ("job_id", "enabled_toolsets", "no_agent", "context_from", "job fire semantics"),
        "update": "Stage 20 binds modeled cronjob target/script to task/schedule metadata and denies prompt-only authority; full job lifecycle remains partial.",
        "test": "Hermes cronjob prompt cannot mint recipient; stale capability replay denies.",
        "confidence": "high",
    },
)
...
def harness_candidate_files(root: Path) -> tuple[Path, ...]:
    files: list[Path] = []
    for path in (root / "tools/run_kill_tests.py", root / "tests" / "test_agent_profile_adapters.py"):
        if path.exists():
            files.append(path)
    ...
    if kill_tests.exists():
        rows.append(
            CoverageRow(
                ...
                likely_hook_point="tools/run_kill_tests.py action construction",
