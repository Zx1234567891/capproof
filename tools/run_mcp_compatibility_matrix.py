...
def compatibility_rows() -> list[dict[str, str]]:
    return [
        row("local stdio MCP server", "supported", "tools/run_capproof_mcp_server.py --stdio", "python tools/run_capproof_mcp_server.py --list-tools", "Local process stdio only."),
        ...
        row("tools/list", "supported", "7 CapProof tools observed", "python tools/run_capproof_mcp_server.py --list-tools", "Also observed in real Hermes smoke."),
        row("tools/call", "supported", "standard CapProof MCP call path", "python tools/run_capproof_mcp_server.py --self-test", "Authority-bearing tools enter guard path."),
        ...
        row("capproof.echo_summary", "supported", "tools/list exposes tool", "python tools/run_capproof_mcp_server.py --list-tools", "No authority-bearing side effect."),
        row("capproof.send_message_mock", "supported", "ALLOW/DENY paths tested", "python tools/run_capproof_mcp_server.py --self-test", "Mock only, no real email."),
