...
- `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest tests/test_hermes_adapter_coverage.py -q`: runs the Stage 19 mock-event tests derived from observed local Hermes surfaces. These tests do not run Hermes, install dependencies, execute tools, connect to gateways/MCP servers, or perform network/shell/email side effects.
