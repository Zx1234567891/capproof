...
def _patch_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(parity, "CONFIG_PATH", integration / "configs" / "opencode.capproof.deepseek.real.jsonc")
    monkeypatch.setattr(parity, "REPORT_PATH", integration / "reports" / "real_opencode_deepseek_parity_report.md")
    monkeypatch.setattr(parity, "SUMMARY_PATH", integration / "reports" / "real_opencode_deepseek_parity_summary.json")
    monkeypatch.setattr(parity, "LIVE_LOG_PATH", integration / "reports" / "real_opencode_deepseek_parity_live.log")
    monkeypatch.setattr(parity, "TRACE_PATH", integration / "traces" / "real_opencode_deepseek_parity_trace.jsonl")
...
def test_require_real_missing_gates_fails(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(parity.smoke, "OPENCODE_BINARY", _fake_opencode(tmp_path))
...
def test_config_uses_env_placeholder_not_key(monkeypatch, tmp_path: Path) -> None:
    ...
    parity.augment_opencode_config_with_auth_env(tmp_path / "auth_queue")
