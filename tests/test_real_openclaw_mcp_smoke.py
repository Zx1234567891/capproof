...
def _patch_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(smoke, "CONFIG_PATH", integration / "configs" / "openclaw.capproof.real.json")
    monkeypatch.setattr(smoke, "REPORT_PATH", integration / "reports" / "real_openclaw_mcp_smoke_report.md")
    monkeypatch.setattr(smoke, "SUMMARY_PATH", integration / "reports" / "real_openclaw_mcp_smoke_summary.json")
    monkeypatch.setattr(smoke, "LIVE_LOG_PATH", integration / "reports" / "real_openclaw_mcp_live.log")
    monkeypatch.setattr(smoke, "TRACE_PATH", integration / "traces" / "real_openclaw_mcp_trace.jsonl")
...
def test_require_real_missing_gates_fails(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(smoke, "OPENCLAW_BINARY", _fake_openclaw(tmp_path))
    for name in smoke.REQUIRED_GATES:
    ...
    assert summary["real_openclaw_smoke_passed"] is False
...
def test_generated_config_uses_env_placeholder_not_key(monkeypatch, tmp_path: Path) -> None:
    ...
    smoke.ensure_dirs()
    smoke.prepare_workspace(smoke.WORKSPACE_DIR)
    smoke.write_openclaw_config(smoke.CONFIG_PATH, smoke.WORKSPACE_DIR)
...
def test_mcp_add_command_uses_standard_server(tmp_path: Path) -> None:
...
def test_trace_analysis_requires_allow_and_deny_evidence() -> None:
    entries = [
        ...
        _entry(
            ...
            {"path": "reports/openclaw_output.txt"},
            "ALLOW",
            executor=True,
            event={"executor": "sandboxed_real", "executed": True},
        ),
...
def test_completion_requires_real_deepseek_call(monkeypatch, tmp_path: Path) -> None:
    _patch_paths(monkeypatch, tmp_path)
    monkeypatch.setattr(smoke, "OPENCLAW_BINARY", _fake_openclaw(tmp_path))
    smoke.ensure_dirs()
    smoke.prepare_workspace(smoke.WORKSPACE_DIR)
